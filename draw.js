...
// ── Coordinate helpers ────────────────────────────────────────────────────────
// Virtual world is 840×560. SX/SY scale into actual canvas pixel dimensions.
function SX(v) { return v / 840 * simW; }
function SY(v) { return v / 560 * simH; }
...
function sw(x1, y1, x2, y2, c, g = true) {
  ...
  sctx.strokeStyle = c;
  sctx.lineWidth = Math.max(1, simW / 700);
  sctx.beginPath();
...
function sdot(x, y, c) {
  ...
  sctx.shadowBlur = 6;
  sctx.beginPath();
  sctx.arc(SX(x), SY(y), Math.max(2, simW / 400), 0, Math.PI * 2);
  sctx.fill();
  sctx.restore();
}
...
function slbl(x, y, t, c, s = 8, a = 'center') {
  sctx.save();
  sctx.fillStyle = c;
  sctx.font = `${Math.max(6, SX(s))}px Share Tech Mono, monospace`;
  sctx.textAlign = a;
...
function sbox(x, y, w, h, t, s2, c, fault = false) {
  ...
  sctx.strokeStyle = fc;
  sctx.lineWidth = Math.max(1, simW / 700 * (fault ? 2 : 1.5));
  sctx.fillStyle = fault ? 'rgba(255,0,0,0.08)' : '#030803';
  sctx.beginPath();
  sctx.roundRect(SX(x - w / 2), SY(y - h / 2), SX(w), SY(h), SX(3));
  sctx.fill();
  sctx.stroke();
  sctx.shadowBlur = 0;
  sctx.fillStyle = fc;
  sctx.font = `bold ${Math.max(6, SX(8))}px Share Tech Mono`;
  sctx.textAlign = 'center';
  sctx.textBaseline = 'middle';
  sctx.fillText(t, SX(x), SY(s2 ? y - 3 : y));
  if (s2) {
    sctx.fillStyle = fault ? '#ff4444' : '#2a5a2a';
    sctx.font = `${Math.max(5, SX(6.5))}px Share Tech Mono`;
    sctx.fillText(s2, SX(x), SY(y + 4));
  }
  if (fault) {
    sctx.fillStyle = '#ff4444';
    sctx.font = `bold ${Math.max(5, SX(6))}px Share Tech Mono`;
...
function ssmd(x, y, lbl, c, fault = false, warn = false) {
  ...
  sctx.strokeStyle = fc;
  sctx.lineWidth = Math.max(1, simW / 700 * (fault ? 2 : 1.2));
  sctx.fillStyle = '#030803';
  sctx.beginPath();
  sctx.roundRect(SX(x - 10), SY(y - 5), SX(20), SY(10), SX(2));
  sctx.fill();
  sctx.stroke();
  // End caps
  sctx.fillStyle = fc + '44';
  sctx.beginPath();
  sctx.roundRect(SX(x - 10), SY(y - 5), SX(4), SY(10), SX(1));
  sctx.fill();
  sctx.beginPath();
  sctx.roundRect(SX(x + 6), SY(y - 5), SX(4), SY(10), SX(1));
  sctx.fill();
  sctx.fillStyle = fc;
  sctx.shadowBlur = 0;
  sctx.font = `bold ${Math.max(5, SX(6.5))}px Share Tech Mono`;
  sctx.textAlign = 'center';
  sctx.textBaseline = 'middle';
...
function sdio(x, y, c, fault = false) {
  ...
  if (fault) {
    sctx.fillStyle = '#ff4444';
    sctx.font = `bold ${Math.max(5, SX(7))}px Share Tech Mono`;
    sctx.textAlign = 'center';
    sctx.textBaseline = 'middle';
...
function sfan(x, y, n, spd) {
  ...
  // Housing
  sctx.beginPath();
  sctx.roundRect(SX(x - 13), SY(y - 18), SX(26), SY(36), SX(2));
  sctx.fill();
  sctx.stroke();
  ...
  for (let i = 0; i < 3; i++) {
    ...
    sctx.fillStyle = fc;
    sctx.beginPath();
    sctx.ellipse(0, SY(-5), SX(2.5), SX(6), 0, 0, Math.PI * 2);
    sctx.fill();
    sctx.restore();
  }
  sctx.restore();
  ...
  for (let i = 0; i < 4; i++) {
    sctx.beginPath();
    sctx.rect(SX(x - 10 + i * 5), SY(y + 14), SX(3.5), SY(4));
    sctx.fill();
  }
  sctx.restore();
...
/**
 * Draws the full simulation schematic.
 * Called every animation frame by sim.js.
 * Reads faultKey, potV, simTick from sim.js globals.
 */
function drawSim() {
  ...
  // Colour helpers — return dead/grey colour when rail is absent
  ...
  // ── PCB background ──────────────────────────────────────────────────────
  sctx.save();
  sctx.fillStyle   = 'rgba(0,12,0,0.5)';
  sctx.strokeStyle = '#1a3a1a';
  sctx.lineWidth   = 2;
  sctx.shadowColor = '#00ff41';
  sctx.shadowBlur  = 12;
  sctx.beginPath();
  sctx.roundRect(SX(30), SY(30), SX(780), SY(500), SX(6));
  sctx.fill();
  sctx.stroke();
  sctx.shadowBlur = 0;
  sctx.restore();
  ...
  sctx.save();
  sctx.strokeStyle = '#1a4a1a';
  sctx.lineWidth = 1;
  sctx.setLineDash([4, 4]);
  sctx.beginPath();
  sctx.moveTo(SX(35), SY(530));
  sctx.lineTo(SX(810), SY(530));
  sctx.stroke();
  sctx.setLineDash([]);
  sctx.restore();
  ...
  // ── 12V bus rail ────────────────────────────────────────────────────────
  const V12Y = 65;
  sw(200, 390, 200, V12Y, pF());
  sw(200, V12Y, 810, V12Y, pF());
  slbl(550, 57, '12V RAIL → FAN PIN 1', pF(), 7);

  // ── 5V bus rail ─────────────────────────────────────────────────────────
  const V5Y = 160;
  sw(680, 300, 680, V5Y, p5());
  sw(200, V5Y, 810, V5Y, p5());
  sdot(680, V5Y, p5());
  slbl(550, 152, '5V RAIL', p5(), 7);

  // ── SATA + BARREL → FUSE ───────────────────────────────────────────────
  sw(120, 100, 200, 100, p12());
  sw(200, 100, 200, 390, p12());
  sdot(200, 100, p12());
  sw(80, 390, 200, 390, p12());
  sdot(200, 390, p12());
  // GND returns
  sw(80, 115, 80, 530, pg());
  sdot(80, 530, '#1a4a1a');
  ...
  // ── FUSE → REG Vin + C1 bypass ─────────────────────────────────────────
  sw(220, 390, 680, 390, pF());
  sw(680, 390, 680, 316, pF());
  sdot(680, 390, pF());
  sw(660, 225, 660, V12Y, pF());
  sdot(660, V12Y, pF());
  sw(660, 237, 660, 530, '#1a4a1a');
  sdot(660, 530, '#1a4a1a');

  // ── REG → 5V + C2 ──────────────────────────────────────────────────────
  sw(680, 284, 680, V5Y, p5());
  sw(660, 180, 660, V5Y, p5());
  sdot(660, V5Y, p5());
  sw(660, 192, 660, 530, '#1a4a1a');
  sdot(660, 530, '#1a4a1a');

  // ── R3 → 5V rail ───────────────────────────────────────────────────────
  sw(610, 180, 610, V5Y, p5());
  sdot(610, V5Y, p5());

  // ── R3 → LED → GND ─────────────────────────────────────────────────────
  sw(610, 174, 610, 145, pg());
  sw(610, 119, 610, 530, pg());
  sdot(610, 530, '#1a4a1a');

  // ── REG → IC555 P8 + P4 ────────────────────────────────────────────────
  sw(630, 300, 380, 300, p5());
  sw(380, 300, 380, 380, p5());
  sdot(380, 300, p5());
  slbl(505, 293, 'Vout→P8/P4', p5(), 6.5);

  // ── R1 → 5V rail + P7 node ─────────────────────────────────────────────
  sw(610, 225, 610, V5Y, p5());
  sdot(610, V5Y, p5());
  ...
  // ── P7 bus line (shared node: R1, R2, D1A all meet here) ────────────────
  sw(490, 350, 615, 350, R1c);
  sdot(490, 350, R1c);
  sdot(610, 350, R1c);
  slbl(550, 342, 'P7 NODE', R1c, 6.5);
  sw(610, 350, 610, 380, R1c);   // P7 → IC555 Pin7
  sdot(615, 350, R1c);

  // ── R2 → D2K discharge path ─────────────────────────────────────────────
  sw(490, 480, 560, 480, R2c);

  // ── D2A → POT right leg ─────────────────────────────────────────────────
  sw(560, 416, 560, 310, iF('D2') ? '#252525' : '#ffdd44');
  sdot(560, 310, '#ffdd44');
  sw(560, 310, 535, 310, iF('D2') ? '#252525' : '#ffdd44');

  // ── POT left+wiper → D1K ────────────────────────────────────────────────
  sw(465, 310, 490, 310, iF('D1') ? '#252525' : '#ffdd44');
  sdot(490, 310, '#ffdd44');
  sw(490, 310, 490, 416, iF('D1') ? '#252525' : '#ffdd44');

  // ── POT wiper → IC555 P2/6 ──────────────────────────────────────────────
  sw(500, 345, 380, 345, pg());
  sw(380, 345, 380, 380, pg());
  sdot(380, 380, pg());
  slbl(440, 337, 'wiper→P2/6', pg(), 6.5);

  // ── C3 → P2/6 ───────────────────────────────────────────────────────────
  sw(660, 390, 660, 380, iF('CT') ? '#333' : '#ff6b00');
  sw(660, 380, 410, 380, iF('CT') ? '#333' : '#ff6b00');
  sw(660, 402, 660, 530, '#1a4a1a');
  sdot(660, 530, '#1a4a1a');
  slbl(535, 371, 'C3→P2/6', iF('CT') ? '#333' : '#ff6b00', 6.5);

  // ── C4 → P5 ─────────────────────────────────────────────────────────────
  sw(710, 390, 710, V5Y, p5());
  sdot(710, V5Y, p5());
  sw(710, 402, 710, 530, '#1a4a1a');
  sdot(710, 530, '#1a4a1a');
  sw(710, 390, 410, 390, p5());
  slbl(560, 381, 'C4→P5', p5(), 6.5);

  // ── P3 → PWM → fans ─────────────────────────────────────────────────────
  sw(350, 420, 350, V12Y + 15, p3c);
  sw(350, V12Y + 15, 810, V12Y + 15, p3c);
  slbl(580, V12Y + 23, 'P3→PWM→Fan Pin4', p3c, 6.5);

  // ── Per-fan 12V + PWM + GND drops ───────────────────────────────────────
  ...
  sctx.beginPath();
  sctx.roundRect(SX(40), SY(85), SX(76), SY(26), SX(2));
  sctx.fill();
  sctx.stroke();
  slbl(78, 98, 'SATA POWER', dead ? '#333' : '#ff6b00', 7);
  sctx.restore();
  ...
  sctx.fillStyle   = '#020802';
  sctx.shadowColor = dead ? '#333' : '#ff6b00';
  sctx.shadowBlur  = dead ? 2 : 10;
  sctx.beginPath();
  sctx.arc(SX(80), SY(420), SX(18), 0, Math.PI * 2);
  sctx.fill();
  sctx.stroke();
  sctx.beginPath();
  sctx.arc(SX(80), SY(420), SX(7), 0, Math.PI * 2);
  sctx.stroke();
  sctx.fillStyle = dead ? '#333' : '#ff6b00';
  sctx.beginPath();
  sctx.arc(SX(80), SY(420), SX(3.5), 0, Math.PI * 2);
  sctx.fill();
  slbl(80, 443, '12V BARREL', dead ? '#333' : '#ff6b00', 6.5);
  sctx.restore();
  ...
  // ── Potentiometer ────────────────────────────────────────────────────────
  sctx.save();
  sctx.strokeStyle = '#2a5a2a';
  sctx.lineWidth   = 1;
  sctx.fillStyle   = '#020802';
  sctx.beginPath();
  sctx.arc(SX(500), SY(310), SX(32), 0, Math.PI * 2);
  sctx.fill();
  sctx.stroke();
  ...
  sctx.shadowBlur  = 6;
  sctx.beginPath();
  sctx.roundRect(SX(482), SY(292), SX(36), SY(36), SX(3));
  sctx.fill();
  sctx.stroke();
  const pang = (135 + potV * 270) * Math.PI / 180;
  sctx.lineWidth = 2;
  sctx.beginPath();
  sctx.moveTo(SX(500), SY(310));
  sctx.lineTo(SX(500) + Math.cos(pang) * SX(14), SY(310) + Math.sin(pang) * SX(14));
  sctx.stroke();
  sctx.fillStyle = '#00ff41';
  sctx.beginPath();
  sctx.arc(SX(500), SY(310), SX(2.5), 0, Math.PI * 2);
  sctx.fill();
  slbl(500, 347, 'RV1 90kΩ', '#2a5a2a', 6.5);
  sctx.restore();
  ...
  sctx.shadowColor = ledOn ? '#00ff41' : '#1a3a1a';
  sctx.shadowBlur  = ledOn ? 12 : 2;
  sctx.beginPath();
  sctx.arc(SX(610), SY(130), SX(8), 0, Math.PI * 2);
  sctx.fill();
  sctx.stroke();
  if (ledOn) {
    sctx.fillStyle = '#00ff41';
    sctx.beginPath();
    sctx.arc(SX(610), SY(130), SX(3), 0, Math.PI * 2);
    sctx.fill();
  }
  slbl(610, 113, 'LED1', ledOn ? '#00ff41' : '#1a3a1a', 6.5);
  sctx.restore();

  // ── SMD passives ─────────────────────────────────────────────────────────
  ssmd(610, 180, 'R3', '#00ff41', iF('R3'));       slbl(610, 194, 'R3 4.7k', '#2a5a2a', 6.5);
  ssmd(610, 225, 'R1', '#44aaff', iF('R1'));       slbl(610, 238, 'R1 1kΩ',  '#2a5a2a', 6.5);
  ssmd(660, 180, 'C2', '#44aaff',
    iF('C2') && f.v5 === 0,
    iF('C2') && f.v5 > 0);                         slbl(660, 194, 'C2 .33µF','#2a5a2a', 6.5);
  ...
  sctx.save();
  sctx.strokeStyle = icc;
  sctx.lineWidth   = Math.max(1, simW / 500);
  sctx.fillStyle   = '#020802';
  sctx.shadowColor = icc;
  sctx.shadowBlur  = ic555fault ? 14 : 8;
  sctx.beginPath();
  sctx.roundRect(SX(350), SY(380), SX(60), SY(80), SX(3));
  sctx.fill();
  sctx.stroke();
  // Notch
  sctx.fillStyle   = '#020802';
  ...
  sctx.lineWidth   = 1;
  sctx.beginPath();
  sctx.arc(SX(380), SY(380), SX(5), 0, Math.PI);
  sctx.fill();
  sctx.stroke();
  slbl(380, 370, 'NE555', icc, 8);
  slbl(380, 361, 'U1',    '#2a5a2a', 6.5);
  if (ic555fault) {
    slbl(380, 420, '✗FAULT', '#ff4444', 7);
  } else {
    [['1',357,388],['2',357,398],['3',357,408],['4',357,418],
     ['8',403,388],['7',403,398],['6',403,408],['5',403,418]
    ].forEach(([t, px, py]) => {
      sctx.save();
      sctx.fillStyle = '#2a5a2a';
      sctx.font = `${Math.max(5, SX(6))}px Share Tech Mono`;
      sctx.textAlign = 'center';
      sctx.textBaseline = 'middle';
      sctx.fillText(t, SX(px), SY(py));
      sctx.restore();
    });
  }
  sctx.restore();
  ...
  sctx.strokeStyle = '#1a4a1a';
  sctx.lineWidth   = 1;
  sctx.strokeRect(SX(720), SY(45), SX(75), SX(40));
  slbl(757, 55, 'UENORTH', '#2a5a2a', 7);
  slbl(757, 65, 'FAN-K11', '#2a5a2a', 8);
  sctx.restore();
